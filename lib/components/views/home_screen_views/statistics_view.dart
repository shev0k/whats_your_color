...
class StatisticsView extends StatefulWidget {
  ...
  const StatisticsView({super.key, required this.animationController});
...
class StatisticsViewState extends State<StatisticsView> {
  ...
  void _connectToWebSocket() {
    channel = WebSocketChannel.connect(
      Uri.parse('wss://whats-your-color.onrender.com'),
    );
  ...
  Widget build(BuildContext context) {
    const imagePath = 'assets/introduction_animation/statistics_paint.png';
    final userText = _generateUserText();
  ...
  String _generateUserText() {
    if (activeUsers == 0) {
      return 'There is no user engaging with the app right now.';
    } else if (activeUsers == 1) {
      return 'There is 1 user engaging with the app right now.';
    } else {
      return 'There are $activeUsers users engaging with the app right now.';
    }
  }
